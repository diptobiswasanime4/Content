    ...
    pub fn new(_c: &EngineState) -> Self {
        Self {
            ldtk_map: LdtkWorldMap::new(
                parse_ldtk_map(include_str!("/assets/comfy_ldtk.ldtk")).unwrap(),
                LDTK_PATH,
            ),
        }
    }
...
fn setup(_state: &mut GameState, c: &mut EngineContext) {
    ...
    c.load_texture_from_bytes(
        "tileset",
        include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/assets/tiles.png")),
    );
}
        ...
        if let Some(_tileset) = tileset {
            ...
            for tile in layer.grid_tiles.iter() {
                ...
                draw_sprite_ex(
                    ...
                    10 + i as i32,
                    DrawTextureParams {
                        dest_size: Some(splat(grid_size / 16.0).as_world_size()),
