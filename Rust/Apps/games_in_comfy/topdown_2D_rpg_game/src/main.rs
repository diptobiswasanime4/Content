...
fn setup(c: &mut EngineContext) {
    ...
    c.load_texture_from_bytes(
        "grass",
        include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/assets/grass.png")),
    );

    // Load the player texture
    c.load_texture_from_bytes(
        "player",
        include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/assets/chicken.png")),
    );
    ...
    for x in 0..50 {
        for y in 0..50 {
            ...
            commands().spawn((
                Sprite::new("grass".to_string(), vec2(1.0, 1.0), 0, WHITE).with_rect(
                    32 * variant,
                    0,
                    32,
                    32,
...
fn update(c: &mut EngineContext) {
    ...
    for (_, (_, animated_sprite, transform)) in world()
